...
const VoucherTable = () => {
  ...
  <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
    ...
    <tbody id="recordGroup">
      ...
      {records.map((record,index)=>(<VoucherTableRow record={record} key={record.product.id} index={index}/>))}
    </tbody>
    <tfoot>
      ...
      <tr className="border-b">
        ...
        <td className="px-6 py-4 text-end" id="recordTotal">{net_total.toFixed(2)}</td>
      </tr>
    </tfoot>
  </table>
</div>
